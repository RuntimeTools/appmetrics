...
Request.prototype.traceStart = function() {
  if (!this.tracedStart) {
    ...
    if (this.parent) {
      ...
      // FIXME(sam) assigning to a global parentId that appears to never be
      // used, but I can't be sure.
      if (this.parent.type) {
...
Request.prototype.traceStop = function() {
  if (!this.traceStopped) {
    ...
    if (
      config.minClockStack != -1 &&
      this.timer.timeDelta >= config.minClockStack
    ) {
      this.stack = this.fetchStack();
    }

    // FIXME(sam) assignment to a local parentId which is never used, so either
    // this whole chunk of code can be deleted, or the `var parentId` is a bug,
    // and this is supposed to be setting a global, except that wouldn't be
    // parallel request safe... not sure what to do here.
    var parentId;
    if (this.parent) {
      if (this.parent.type) {
        parentId = this.parent.id;
...
Request.prototype.stop = function(context) {
  if (this.active) {
    ...
    if (
      config.minClockTrace != -1 &&
      this.timer.timeDelta >= config.minClockTrace
    ) {
...
function isNotDeepDiveStack(line) {
  return (
    line.indexOf('appmetrics') == -1 && line.indexOf('lib/aspect.js') == -1
  );
}
