...
HttpsProbe.prototype.metricsEnd = function(probeData, method, url, res, httpsReq) {
    if(probeData && probeData.timer) {
	    ...
	    am.emit('https', {time: probeData.timer.startTimeMillis, method: method, url: url, duration: probeData.timer.timeDelta, header: res._header, statusCode: res.statusCode, contentType: res.getHeader('content-type'), requestHeader: httpsReq.headers});
    }
};
...
HttpsProbe.prototype.requestEnd = function (probeData, method, url, res, httpsReq) {
    if(probeData && probeData.req)
        probeData.req.stop({url: url, method: method, requestHeader: httpsReq.headers, statusCode: res.statusCode, header: res._header, contentType: res.getHeader('content-type')});
};
